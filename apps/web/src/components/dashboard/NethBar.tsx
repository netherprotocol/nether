...
import { AddressLink, StatRow } from './ui.tsx';
    ...
    <section className="rounded-xl border border-white/10 bg-[#0c0c0c] px-5 py-4 md:px-6">
      <header className="flex items-start gap-3 md:hidden">
        ...
        >
          ...
          <ChevronDown
            className={['mt-0.5 h-5 w-5 text-muted transition-transform', open ? 'rotate-180' : ''].join(' ')}
            strokeWidth={1.5}
            aria-hidden="true"
          />
        </button>
      ...
      {open ? (
        <div className="mt-4 md:hidden">
          <div className="divide-y divide-white/10">
            <StatRow label="Total supply">{snapshot ? formatNeth(snapshot.nethSupply) : '—'}</StatRow>
            <StatRow label="Your balance">{showBalance ? formatNeth(nethBalance) : '—'}</StatRow>
