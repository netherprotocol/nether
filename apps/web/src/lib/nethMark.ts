export const NETH_MARK_VIEWBOX = '0 0 1254 1254';
...
const DEFAULT_SITE = 'https://rastsislaux.github.io';
...
export function nethMarkUrl(): string {
  const originRaw = envString('SITE') ?? DEFAULT_SITE;
  const origin = originRaw.endsWith('/') ? originRaw : `${originRaw}/`;
  const baseRaw = envString('BASE_URL') ?? DEFAULT_BASE;
