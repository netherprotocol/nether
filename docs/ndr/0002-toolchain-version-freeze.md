...
This NDR names the versions to freeze. Frontend, indexer, and Gravekeeper stacks are out of scope (frontend: [`NDR-0003`](0003-frontend-stack.md); indexer and keeper remain later NDRs).
...
Not in this freeze: frontend (see [`NDR-0003`](0003-frontend-stack.md)), indexer, Gravekeeper, AAVE adapter addresses, or optimizer_runs retunes after a measured W6 size/gas problem (that retune would supersede this NDR if it must change the frozen compiler settings).
