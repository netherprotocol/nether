...
|---|---|---|---|
| 1 | Scaffold Solidity / OpenZeppelin / toolchain | **W0** | Unblocks everything else |
...
2. **Test invest adapter** (approved split): idle ETH or a scripted profit/loss adapter for unit, fuzz, and invariant tests so M0 does not depend on AAVE. Do not deploy it on mainnet (§18.2).
...
| This plan | Living document. Adjust on demand. Do not copy it into an NDR. |
...
Do not open these until the question is actually being decided.
